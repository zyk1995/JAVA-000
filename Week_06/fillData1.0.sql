...
BEGIN
    ...
    WHILE i<=100 DO
        set autocommit = 1;
        insert into zyk_users (name, password, phoneNumber, identify_card, money)
...
BEGIN
    ...
    set autocommit=0;
    WHILE i<=100 DO
            insert into zyk_stores (name, description) VALUES (CONCAT('store', i), CONCAT('description', i));
            SET i = i+1;
        END WHILE;
    commit;
END $
...
BEGIN
    ...
    set autocommit=0;
    WHILE i<=10000 DO
        insert into zyk_goods(name, description, price, weight, store_id, store_name, status)
...
BEGIN
    ...
    set autocommit=0;
    WHILE i<=1000000 DO
            insert into zyk_orders (user_id, commodities, status, deliver_status, total_price, create_time, update_time)
